...
vector<vector<int>> all_car_path(vector<vector<int>> map_array, vector<vector<int>> map_road_array, vector<vector<int>> car_inf, vector<int> car_time)
{
...
vector<int> creat_car_time(vector<vector<int>> car_inf, int car_per_sec)
{
    int max_speed = 0;
    int n = (int)car_inf.size();
    vector<int> car_time(30000);
    vector<int> car_list;
    ...
    for (int i = max_speed -1; i >= 0; --i) {
        time += 1;
        cur_group = car_divide_speed[i];
        if(cur_group.empty()) continue;
        cur_amount = (int)cur_group.size();
        for (int j = 0; j < cur_amount; ++j) {
            car_time[cur_group[j]] = time;
            if (!((j + 1) % car_per_sec)) time += 1;
        }
    }
    return car_time;
}
...
int main(int argc, char *argv[])
{
    ...
    vector<int> car_time = creat_car_time(car_info_all, car_per_s);
    vector<vector<int>> answer = all_car_path(dis_array, road_array, car_info_all, car_time);
